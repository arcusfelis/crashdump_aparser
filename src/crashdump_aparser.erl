...
%%%-----------------------------------------------------------------
%%% Start crashdump_parser via the cdv script located in
...
get_filename() ->
    call(get_filename).
parse_limited(Line1, Limit) ->
    call({parse_limited, Line1, Limit}).
calc_limited_length(Line1) ->
    calc_limited_length(Line1, 0).

calc_limited_length(Line1, Len) ->
    Limit = 10,
    {ok, List} = parse_limited(Line1, Limit),
    calc_limited_length1(List, Len).

calc_limited_length1([_|T], Len) ->
    calc_limited_length1(T, Len+1);
calc_limited_length1([], Len) ->
    Len;
calc_limited_length1({'#CDVLimitReached', Line1}, Len) ->
    calc_limited_length(Line1, Len).

%%%-----------------------------------------------------------------
%%% Called when a link to a process (Pid) is clicked.
proc_details(Pid) ->
    call({proc_details,Pid, undefined}).

proc_details(Pid, Limit) ->
    call({proc_details,Pid, Limit}).
...
%%--------------------------------------------------------------------
init([]) ->
    skipinttab:new(cdp_memory),
    (catch ets:setopts(cdp_memory, {heir, erlang:list_to_pid("<0.0.0>"), xxx})),
...
handle_call({proc_details,Pid,Limit},_From,
...
handle_call(get_filename, _From,State=#state{file=File}) ->
    {reply,{ok,File},State};
handle_call({parse_limited, Line1, Limit},_From,State=#state{file=File, dump_vsn=DumpVsn}) ->
    Fd = open(File),
    put(fd, Fd),
    put(fd2, Fd),
    BinAddrAdj = get_bin_addr_adj(DumpVsn),
    {Term, _, _} = parse_limited(Line1, Limit, BinAddrAdj),
    close(Fd),
    {reply,{ok,Term},State}.
...
%%% get_progress/0 until it returns {ok,done}.
do_read_file(File) ->
...
expand_memory(Fd,Pid,DumpVsn,Binaries) ->
    BinAddrAdj = get_bin_addr_adj(DumpVsn),
    put(fd,Fd),
    case ets:member(cdp_memory, Pid) of
        true -> put(fd2, Fd);
        false -> ok
    end,
    Dict = case (catch ets:member(cdp_memory, Pid)) of
               true ->
                   gb_trees:empty(); %% done memory map before
               _ ->
                   io:format("read_heap~n", []),
                   read_heap(Fd,Pid,BinAddrAdj,Binaries) %% regular parse
           end,
    io:format("read_stack_dump~n", []),
    SD = read_stack_dump(Fd,Pid,BinAddrAdj,Dict),
    io:format("read_messages~n", []),
    Ms = read_messages(Fd,Pid,BinAddrAdj,Dict),
    io:format("read_dictionary~n", []),
    PD = read_dictionary(Fd,Pid,BinAddrAdj,Dict),
    Expanded = {SD, Ms, PD},
    erase(fd),
    Expanded.
...
read_stack_dump1(Fd,BinAddrAdj,Dict,Acc) ->
	...
	Line ->
        put(fd2, Fd),
        {ok, OldPos} = file:position(Fd, {cur,0}),
        io:format("TOOOP Line0 ~p, OldPos ~p~n", [Line, OldPos]),

	    {Stack, D1} = parse_top(Line,BinAddrAdj,Dict),
	    read_stack_dump1(Fd,BinAddrAdj,D1,[Stack|Acc])
    end.

parse_top(Line0, BinAddrAdj, D) ->
    {Label,Line1} = get_label(Line0),
    %% arcusfelis: there was 
%   {Term,Line,D} = parse_term(Line1, BinAddrAdj, D),
    {Term,Line,D1} = parse_term(Line1, BinAddrAdj, D),
%   {Term,Line,D1} = parse_heap_term(Line1, Label, BinAddrAdj, D),
    [] = skip_blanks(Line),
    {{Label,Term}, D1}.
...
parse(Line0, BinAddrAdj, Dict0) ->
    ...
    %% because of cdp_memory, we can parse the same address several times
    %% check Dict0 to avoid it
    case gb_trees:is_defined(Addr, Dict0) of
	    true -> Dict0;
	    false -> parse0(Addr, Line1, BinAddrAdj, Dict0)
	end.

parse0(Addr, Line1, BinAddrAdj, Dict0) ->
...
level_up(D) ->
    case gb_trees:lookup(limits, D) of
        none ->
            case get(limit) of
                undefined ->
                    D;
                DefLimit ->
                    gb_trees:insert(limits, [DefLimit], D)
            end;
        {value, Limits} ->
            DefLimit = get(limit),
            gb_trees:update(limits, [DefLimit|Limits], D)
    end.
...
level_down(D) ->
    case gb_trees:lookup(limits, D) of
        none ->
            D;
        {value, [_]} ->
            gb_trees:delete(limits, D);
        {value, [_|Limits]} ->
            gb_trees:update(limits, Limits, D)
    end.
...
limit_reached(D) ->
    case gb_trees:lookup(limits, D) of
        ...
        {value, [0|Limits]} ->
            {true, D};
        {value, [Limit|Limits]} ->
            D1 = gb_trees:update(limits, [Limit-1|Limits], D),
            {false, D1}
    end.
...
%%%
parse_heap_term([$l|Line0], Addr, BinAddrAdj, D00) ->	%Cons cell.
    case limit_reached(D00) of
        {false, D0} ->
            {H,"|"++Line1,D1} = parse_term(Line0, BinAddrAdj, level_up(D0)),
            {T,Line,D2} = parse_term(Line1, BinAddrAdj, level_down(D1)),
            Term = [H|T],
            D = gb_trees:insert(Addr, Term, D2),
            {Term,Line,D};
        {true, D0} ->
            {H,"|"++Line1,D1} = parse_term(Line0, BinAddrAdj, level_up(D0)),
            T = {'#CDVLimitReached', Line1},
            % Use to decode:
%           {Term,Line,D2} = parse_term(Line1, BinAddrAdj, D1),
            Term = [H|T],
            D = gb_trees:insert(Addr, Term, level_down(D1)),
            {Term,"",D}
    end;
...
parse_heap_term("Ys"++Line0, Addr, BinAddrAdj, D0) ->	%Sub binary.
    ...
    Term = case gb_trees:lookup(Binp, D0) of
		   ...
		   case gb_trees:lookup(Binp0, D0) of
		       {value,Bin} -> cdvbin(Offset,Sz,Bin);
		       none -> {'#CDVNonexistingBinary', "Ys" ++ Line0}
		   end;
	       none -> {'#CDVNonexistingBinary', "Ys" ++ Line0}
	   end,
    D = gb_trees:insert(Addr, Term, D0),
...
deref_ptr(Ptr, Line, BinAddrAdj, D0) ->
	...
	none ->
        case get(fd2) of
            undefined ->
                deref_ptr_without_memory_map(Ptr, Line, BinAddrAdj, D0);
            Fd ->
                deref_ptr_with_memory_map(Fd, Ptr, Line, BinAddrAdj, D0)
        end
    end.
...
deref_ptr_with_memory_map(Fd, Ptr, Line, BinAddrAdj, D0) ->
    ...
    case maybe_move_to(Fd, Ptr) of
        true ->
            ...
            %% direct read
            L = val(Fd),
            D = try parse(L, BinAddrAdj, D0) catch C:R -> S = erlang:get_stacktrace(), erlang:raise(C, {R, L}, S) end,
            restore_state(State),
            {ok, OldPos} = file:position(Fd, OldPos),
            deref_ptr(Ptr, Line, BinAddrAdj, D);
        false ->
            {['#CDVNotFound'],Line,D0}
    end.

maybe_move_to(Fd, Ptr) ->
    case skipinttab:lookup(cdp_memory, Ptr) of
        undefined -> false;
        Pos -> {ok, _} = file:position(Fd, -Pos), true
    end.

%   case (catch ets:lookup(cdp_memory, Ptr)) of
%       [{_, Pos}] -> {ok, _} = file:position(Fd, Pos), true;
%       _ -> false
%   end.
...
make_memory_map(Pid) ->
    case (catch ets:member(cdp_memory, Pid)) of
        true -> ok;
        _ -> force_make_memory_map(Pid), ets:insert(cdp_memory, {Pid}), ok
    end.

force_make_memory_map(Pid) ->
    {ok, File} = get_filename(),
    [{_,Start}] = lookup_index(proc_heap, Pid),
    End = hd(lists:sort(ets:select(cdv_dump_index_table,[{{{'_','$1'},'_'},[{'>', '$1', Start}],['$1']}]))),
    case (End-Start) > 10000000 of
        true ->
            open_and_read_vals_parallel(File, Start, End, 8);
        false ->
            open_and_read_vals_parallel(File, Start, End, 1) %% too small, one thread
    end.

open_and_read_vals_parallel(File, Start, End, Parts) ->
%   Tab = ets:new(cdp_memory, [ordered_set, named_table, public, {write_concurrency, true}]),
    Tab = skipinttab:new(cdp_memory),
%   {ok, Tab} = eleveldb:open("cdp_memory", [{create_if_missing,true},{compression,true},{write_buffer_size, 16777216*8}]),
    PartSize = (End - Start) div Parts,
    Starts = [(Start + (Part-1)*PartSize) || Part <- lists:seq(2, Parts)],
    CorrectedStarts = [begin {ok, Fd} = file:open(File, []),
                             {ok, _} = file:position(Fd, S),
                             file:read_line(Fd),
                             {ok, Pos} = file:position(Fd, {cur,0}),
                             file:close(Fd), Pos end || S <- Starts],
    StartEnds = lists:zip([Start|CorrectedStarts], CorrectedStarts ++ [End]),
    PidRefs = [spawn_monitor(fun() -> open_and_read_vals_until(File, Tab, Start, End) end) || {Start, End} <- StartEnds],
    wait_for_monitors(PidRefs),
    Tab.

wait_for_monitors([{_Pid,Ref}|PidRefs]=All) ->
    receive
        {'DOWN', Ref, _, _, _} ->
            wait_for_monitors(PidRefs)
    after 10000 ->
        Pids = [P || {P,_} <- All],
        io:format("Waiting for ~p, size=~p~n", [Pids, ets:info(cdp_memory, size)]),
        wait_for_monitors(All)
    end;
wait_for_monitors([]) ->
    [].


open_and_read_vals_until(File, Tab, Start, End) ->
    {ok, Fd} = file:open(File, [{read_ahead, 16777216*8}, raw, binary, read]),
    file:position(Fd, Start),
    {Time,_} = timer:tc(fun() -> read_vals_until(Fd, Tab, Start, End, skipinttab:new_bulk_write()) end),
    file:close(Fd),
    io:format("Done ~p ~p~n", [self(), Time]),
    ok.

read_vals_until(Fd, Tab, Start, End, Bulk) when Start >= End ->
    {ok, Cur} = file:position(Fd, cur),
    skipinttab:bulk_flush(Tab, Bulk),
    io:format("Cur=~p, Start=~p, Diff=~p~n", [Cur, Start, Cur-Start]),
    Cur = Start,
    [];
read_vals_until(Fd, Tab, Start, End, Bulk) ->
    {ok, Bin} = file:read_line(Fd),
    NewPos = Start + byte_size(Bin),
    try parse_address(Bin) of
    Addr ->
        Bulk2 = skipinttab:bulk_write(Tab, Addr, -Start, Bulk),
    %   ets:insert_new(Tab, {Addr, binary:copy(ValueBin)}),
    %   eleveldb:put(Tab, address_to_binary(Addr), position_to_binary(Start), []),
        read_vals_until(Fd, Tab, NewPos, End, Bulk2)
    catch _:_ ->
        io:format("Failed to parse ~p, pid=~p~n", [Bin, self()]),
        {ok, Cur} = file:position(Fd, cur),
        skipinttab:bulk_flush(Tab, Bulk),
        io:format("Cur=~p, Start=~p, Diff=~p~n", [Cur, Start, Cur-Start]),
        []
...
parse_address(Bin) ->
    [AddrBin, ValueBin] = binary:split(Bin, <<":">>, []),
    binary_to_integer(AddrBin, 16).

address_to_binary(X) ->
    <<X:64/big>>.

position_to_binary(X) ->
    <<X:64/big>>.

parse_limited(Line1, Limit, BinAddrAdj) ->
    D = gb_trees:empty(),
    parse_limited(Line1, Limit, BinAddrAdj, D).

parse_limited({'#CDVLimitReached', Line1}, Limit, BinAddrAdj, D) ->
    parse_limited(Line1, Limit, BinAddrAdj, D);
parse_limited(Line1, Limit, BinAddrAdj, D) ->
    put(limit, Limit),
    {Term,Line,D1} = parse_term(Line1, BinAddrAdj, level_up(D)),
    erase(limit),
    {Term,Line,level_down(D1)}.


read_binary({'#CDVNonexistingBinary', Line}) ->
    read_binary(Line);
read_binary("Yc"++Line0) ->	%Reference-counted binary.
    {BinpStr,Line1} = get_string(Line0),
    {Offset,":"++Line2} = get_hex(Line1),
    {Size,Line} = get_hex(Line2),
    [{_, FilePos}] = lookup_index(binary, BinpStr),
    {ok, File} = get_filename(),
    {ok, Fd} = file:open(File, []),
    try
        {ok, _} = file:position(Fd, FilePos),
        {ok, BinLine} = file:read_line(Fd),
        {Bin, _} = get_binary(Offset,Size,BinLine),
        Bin
    after
        file:close(Fd)
    end.
